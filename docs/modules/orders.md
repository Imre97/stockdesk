...
- Modifying an order recomputes its reservation and re-checks buying power (against buying power plus the order's own current reservation) and the margin deficit rule: a modify that increases a position while the account is flagged returns `422 MARGIN_DEFICIT`. Every modify pushes one `order_update` before re-evaluation and an `account_summary`. A reservation is otherwise frozen at placement or modification: fills of neighbouring orders or price moves do not recompute it. Tests recompute a `MARKET` reservation from the last price they emitted before placement.
...
13. Orders tab details (Phase C2): child rows link to the parent by opening its details drawer (no per-order route); the symbol page Orders tab lists every status of the symbol for the active account; the symbol text filter is a case-insensitive prefix match, forwarded to the API only when it parses as a symbol; a `TRIGGERED` plain `STOP` still exposes `stopPrice` for modification. PATCH and DELETE responses `{ order }` are parsed on the web without a dedicated shared envelope schema; adding `orderResponseSchema` to `packages/shared/src/order-responses.ts` is open.
