...
20. WebSocket subscribe messages that refuse some symbols add the ones that fit and send `SUBSCRIPTION_LIMIT` once; unknown symbols get one `SYMBOL_NOT_FOUND` each. Message handling is serialized per socket. The wire schema accepts up to `QUOTE_MESSAGE_SYMBOL_CAP = 100` symbols per message (a sanity cap); the per-socket limit of 50 is enforced only by the subscription table.
21. Bucket completeness and cache windows use the calendar stepper (`nextBucketStartMs`, `windowStartMs`), never a fixed millisecond duration; a provider bar is stored final exactly when its bucket has ended. Profile and metrics refresh independently (`getProfile(symbol, { parts })`): a young profile with stale metrics refreshes only the metrics endpoint.
22. Coverage never extends past what was stored: after a provider fetch the covered interval ends at `bucketStartMs(min(end, now), timeframe)`, the start of the bucket that is still forming, and the cache-hit rule compares against the same clipped instant, so a repeated request inside one bucket is a hit and the first request after the bucket closes fetches it. A request window is capped at the provider's history depth (2 years of `1D`, 30 days of `1m`), so a `1M` request with the default limit never asks for 25 years.
23. For a closed bucket the provider is authoritative: a provider fetch upserts over any row the live aggregator wrote for the same `(symbol, timeframe, time)` (forming or final), so a partial session aggregate or a row left `isFinal = false` by a shutdown is replaced; a row is never left non-final for a bucket the provider has served. The history depth used to cap request windows is a provider property (`historyDepth` on `MarketDataProvider`; simulated: 2 years daily, 30 days intraday; Alpaca: its plan's limits), applied for the provider that serves `bars`.
