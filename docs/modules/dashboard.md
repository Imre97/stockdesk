...
- Deposit amount: greater than 0, at most 2 decimal places, at most `1000000.00` per deposit (`DEPOSIT_LIMIT_EXCEEDED`). The ceiling is the shared constant `DEPOSIT_LIMIT` (a `Decimal` equal to `1000000.00`) in `packages/shared`, not an environment variable, so `depositSchema` enforces it on both sides.
...
- Tests drive the jobs with an injected `now: () => Date` and direct `runSnapshotTick()` / `runThinning()` calls. No fake timers for the tick and thinning logic; the only fake-timer test is the `start()` / `stop()` scheduling test.
...
- `POST /accounts` writes a snapshot for every account of the user (idempotent through `skipDuplicates`, so the existing accounts simply gain a fresh point) and broadcasts `account_summary` after the transaction commits; registration writes one snapshot after commit through the same path deposits use. A failing snapshot is logged and never fails the parent operation.
...
- API integration: criteria 1 to 6, 14, 15. The jobs are driven by direct `runSnapshotTick()` / `runThinning()` calls with an injected `now: () => Date`, with fake timers used only for the `start()` / `stop()` scheduling test. The ledger invariant `cashBalance == latest balanceAfter` (ordering `(createdAt desc, id desc)`) is asserted after each cash operation. The daily P&L reference is tested across the DST days 2026-03-08 and 2026-11-01.
