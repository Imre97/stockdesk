...
- End-to-end: Playwright in its own workspace `apps/e2e`, one spec per user-visible flow, against the dedicated `stockdesk_e2e` database (`DATABASE_URL_E2E`) so it never shares state with the unit test or development databases. The suite has no `test` script, so `npm test` and the test hook skip it; the config starts the API and the Vite dev server itself, the API web server command runs `prisma migrate deploy` and truncates every table before the server boots (so the boot-time symbol seed survives), and the Playwright global setup waits until `GET /api/v1/health` reports `ready: true`.
